...
class Dcam:
    ...
    def buf_getframe_with_timestamp_and_framestamp(self, iFrame):
        """Return DCAMBUF_FRAME instance with timestamp and framestamp.
        Return DCAMBUF_FRAME instance with image data, timestamp, and framestamp specified by iFrame.

        Arg:
            iFrame (int): Index of target frame.

        Returns:
            (aFrame, npBuf, timestamp, framestamp): 
                - aFrame: DCAMBUF_FRAME
                - npBuf: NumPy buffer
                - timestamp: DCAM_TIMESTAMP
                - framestamp: int (Frame stamp)
            bool: False if error happens. lasterr() returns the DCAMERR value.
        """
        if not self.is_opened():
            return self.__result(DCAMERR.INVALIDHANDLE)  # instance is not opened yet.
        ...
        framestamp = aFrame.framestamp  # Retrieve the frame stamp

        return (aFrame, npBuf, timestamp, framestamp)

        
    def buf_getframe_with_timestamp(self, iFrame):
        """Return DCAMBUF_FRAME instance with timestamp.
        Return DCAMBUF_FRAME instance with image data and timestamp specified by iFrame.

        Arg:
            iFrame (int): Index of target frame.

        Returns:
            (aFrame, npBuf, timestamp): aFrame is DCAMBUF_FRAME, npBuf is NumPy buffer, timestamp is DCAM_TIMESTAMP.
            bool: False if error happens. lasterr() returns the DCAMERR value.
        """
        if not self.is_opened():
            return self.__result(DCAMERR.INVALIDHANDLE)  # instance is not opened yet.
